...
function scene:create( event )
   ...
   -- background
   local grad = {type="gradient", color1={0,0,0}, color2={1,.5,.5}, direction="down"}	-- create gradient background
   ...
   self:add_new_object("treefk", globals.image.tree_trunk, 950, 975, {"static", {friction=.5, bounce=0}})   
   self:add_new_object("treefk2", globals.image.tree_trunk, 1050, 850, {"static", {friction=.5, bounce=0}})
   ...
   self:add_new_object("tree6", globals.image.tree_trunk, 150, 775, ground_physics) 
		self.tree6.rotation = 215
...
function scene:process()
	...
	-- animate scene
	local rot_speed = 50
	local rot_delta = (self.tree_timer.delta / 1000) * rot_speed

	self.tree6.rotation = self.tree6.rotation + rot_delta
	self.treefk.rotation = self.treefk.rotation + rot_delta
	self.treefk2.rotation = self.treefk2.rotation + (rot_delta * 1.2)
	
	if self.tree6.rotation > 360 then self.tree6.rotation = 0 end
	if self.treefk.rotation > 360 then self.treefk.rotation = 0 end
	if self.treefk2.rotation > 360 then self.treefk2.rotation = 0 end
